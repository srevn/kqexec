...
watchref_t registry_add(registry_t *registry, struct watch *watch) {
    ...
    watchref_t ref = { watch_id, generation };
...
struct watch *registry_get(registry_t *registry, watchref_t ref) {
    if (!registry || !watchref_valid(ref)) {
        return NULL;
    }
    ...
    watch_t *watch = NULL;
    if (ref.watch_id < registry->capacity &&
        registry->generations[ref.watch_id] == ref.generation &&
        registry->states[ref.watch_id] == WATCH_STATE_ACTIVE) {
        watch = registry->watches[ref.watch_id];
    }
...
bool registry_valid(registry_t *registry, watchref_t ref) {
    ...
    bool valid = (ref.watch_id < registry->capacity &&
                  registry->generations[ref.watch_id] == ref.generation &&
                  registry->states[ref.watch_id] == WATCH_STATE_ACTIVE);
...
static void registry_notify(registry_t *registry, watchref_t ref) {
    ...
    for (int j = 0; j < i; j++) {
        if (snapshot[j] && snapshot[j]->on_watch_deactivated) {
            snapshot[j]->on_watch_deactivated(ref, snapshot[j]->context);
        }
    }
...
void registry_deactivate(registry_t *registry, watchref_t ref) {
    ...
    watch_t *watch = registry->watches[ref.watch_id];
    log_message(DEBUG, "Deactivating watch '%s' (ID: %u, Gen: %u)", 
                watch ? watch->name : "unknown", ref.watch_id, ref.generation);

    /* Phase 1: Notify all observers (releases and reacquires lock) */
    registry_notify(registry, ref);

    /* Phase 2: Mark inactive and increment generation */
    registry->states[ref.watch_id] = WATCH_STATE_INACTIVE;
    registry->generations[ref.watch_id]++;
    registry->count--;
